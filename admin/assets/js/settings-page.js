...
jQuery( document ).ready( function( $ )
{
    ...
    $( '.pml-add-defaults-button' ).on( 'click', function()
    {
        ...
        let defaultsToAdd = [];
        if ( dataType === 'user_agents' && pml_admin_params.default_bot_user_agents )
        {
            defaultsToAdd = pml_admin_params.default_bot_user_agents;
        } else if ( dataType === 'domains' && pml_admin_params.default_bot_domains )
        {
        ...
        if ( newContent.length > 0 )
        {
            ...
            { // If textarea already has content
