...
// Ensure wp_kses is available while keeping production fast and debug friendly.
...
$pml_debug_notice_cookie = 'pml_debug_notice_dismissed';
...
if (
    ...
    is_admin() &&
    ! isset( $_COOKIE[ $pml_debug_notice_cookie ] )
) {
...
if ( ! function_exists( 'pml_debug_mode_admin_notice' ) ) {
    function pml_debug_mode_admin_notice(): void {
        global $pml_debug_notice_cookie;

        if ( isset( $_COOKIE[ $pml_debug_notice_cookie ] ) ) {
            return;
        }

        $message = 'Protect Media Links: WordPress debug mode is enabled. This can slow down protected media responses and harm SEO. Disable debug mode when you are finished troubleshooting.';

        if ( function_exists( 'esc_html__' ) ) {
            $message = esc_html__(
                'Protect Media Links: WordPress debug mode is enabled. This can slow down protected media responses and harm SEO. Disable debug mode when you are finished troubleshooting.',
                'protect-media-links'
            );
        } else {
...
if ( ! function_exists( 'pml_debug_mode_admin_notice_script' ) ) {
    function pml_debug_mode_admin_notice_script(): void {
        ...
        $cookie_js_value = $pml_debug_notice_cookie;

        if ( function_exists( 'wp_json_encode' ) ) {
            $cookie_js_value = wp_json_encode( $cookie_js_value );
        } else {
            $cookie_js_value = "'" . addslashes( $cookie_js_value ) . "'";
        }
        ?>
        <script>
